...
import org.fox.o2o.dao.ProductImgDao;
...
public class ProductServiceImpl implements ProductService {
	...
	private void addProductImgList(Product product,List<ImageHolder> productImgHolderList) {
		//根据product获取相应的店铺id，继而获取图片对应相应店铺的的存储路径
		String dest = PathUtil.getShopImagePath(product.getShop().getShopId());
		List<ProductImg> productImgList = new ArrayList<ProductImg>();
		//遍历图片一次去处理，并添加到productImg实体类
		for(ImageHolder productImageHolder:productImgHolderList) {
			String imgAdrr = ImageUtil.generateNormalImg(productImageHolder, dest);
			ProductImg productImg = new ProductImg();
			productImg.setImgAddr(imgAdrr);
			productImg.setProductId(product.getProductId());
			productImg.setCreateTime(new Date());
			productImgList.add(productImg);
		}
		//如果确认有图片需要添加的，就执行批量添加操作
	...
	public ProductExecution getProductList(Product productCondition, int pageIndex, int pageSize) {
		//页码转换为数据库的行码、并调用dao层返回指定页码的商品列表
