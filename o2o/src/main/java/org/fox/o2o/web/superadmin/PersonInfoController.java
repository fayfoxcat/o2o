...
import org.fox.o2o.entity.ConstantForSuperAdmin;
...
public class PersonInfoController {
	...
	private Map<String, Object> listPersonInfos(HttpServletRequest request) {
		...
		int pageIndex = HttpServletRequestUtil.getInteger(request,
				ConstantForSuperAdmin.PAGE_NO);
		int pageSize = HttpServletRequestUtil.getInteger(request,
				ConstantForSuperAdmin.PAGE_SIZE);
		if (pageIndex > 0 && pageSize > 0) {
			try {
				PersonInfo personInfo = new PersonInfo();
				int enableStatus = HttpServletRequestUtil.getInteger(request,
						"enableStatus");
				if (enableStatus > -1) {
			...
			modelMap.put("success", false);
			modelMap.put("errorMessage", "空的查询信息");
			return modelMap;
		}
	}
		...
		if (userId > 0 && enableStatus > 0) {
			try {
				...
				if (ae.getState() == PersonInfoStateEnum.SUCCESS.getState()) {
					...
					modelMap.put("success", false);
					modelMap.put("errorMessage", ae.getStateInfo());
				}
			...
			modelMap.put("success", false);
			modelMap.put("errorMessage", "请输入需要修改的帐号信息");
		}
		return modelMap;
	}
